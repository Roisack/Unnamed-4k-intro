...
"color.g = (color.r-sqrt((i*i)+(j*j)) * sin(time*0.01));"
"color.b = (abs(sin(color.r * color.g)) * cos(time*0.02));"
"gl_FragColor = color;}";
const char* interference_vs = 
"varying vec2 texcoord;"
"void main()"
"{ vec4 fragmentPos = gl_ModelViewMatrix * gl_Vertex; texcoord = gl_MultiTexCoord0.st; gl_Position = gl_ProjectionMatrix * fragmentPos;"
"}";
